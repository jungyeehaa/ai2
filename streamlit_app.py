...
# Google Drive 파일 ID
file_id = '15GfvqaNdokMFJxP_4dt9B7uPEK1_kKUl'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/nf4DmGN/photo-1694661465950-f2e42e437e29.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=4jH3MqokNMw", start_time=0)
            st.caption(f"유튜브: {label}")

    # 3rd Row - Text based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.write(f"{label} 정말 너무 피곤해서 자고 싶어요.")
